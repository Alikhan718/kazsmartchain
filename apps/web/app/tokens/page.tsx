...
export default function TokensPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Token Hero Card */}
      <div className="animate-fade-in-up stagger-1 relative p-8 rounded-3xl glass-strong border border-amber-500/20 overflow-hidden">
        {/* Background effects */}
        ...
        <div className="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center border-4 border-gray-900">
                <Check className="w-4 h-4 text-white" />
              </div>
            </div>
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h2 className="text-3xl font-bold text-white">KSC Token</h2>
                <span className="px-3 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  ERC-20
                </span>
              </div>
              <p className="text-gray-400 mb-3">KazSmartChain Native Utility Token</p>
              {balances?.token?.address && (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-mono-code text-gray-500">
          ...
          <div className="flex flex-wrap gap-4">
            <div className="p-5 rounded-2xl bg-gray-900/60 border border-gray-800/50 min-w-[140px]">
              <div className="text-xs text-gray-500 mb-1 flex items-center gap-1">
                <TrendingUp className="w-3 h-3" /> Total Supply
              </div>
              <div className="text-3xl font-bold text-amber-400 animate-count">
            ...
            <div className="p-5 rounded-2xl bg-gray-900/60 border border-gray-800/50 min-w-[140px]">
              ...
              <div className="text-3xl font-bold text-cyan-400 animate-count">
                {statsLoading ? '...' : stats?.holders || '0'}
              </div>
              <div className="text-xs text-gray-500">организаций</div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-gray-900/60 border border-gray-800/50 min-w-[140px]">
              ...
              <div className="text-3xl font-bold text-purple-400 animate-count">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in-up stagger-2">
        <div className="p-5 rounded-2xl glass border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 to-emerald-600/5">
          ...
          <div className="text-xs text-gray-500 mt-1">{stats?.mintCount || 0} операций</div>
        </div>
        
        <div className="p-5 rounded-2xl glass border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-cyan-600/5">
          <div className="flex items-center gap-2 mb-3">
            <ArrowUpRight className="w-5 h-5 text-cyan-400" />
            <span className="text-sm text-gray-400">Transferred</span>
          </div>
          <div className="text-2xl font-bold text-cyan-400">
            {statsLoading ? '...' : stats?.totalTransferred || '0'}
          </div>
          <div className="text-xs text-gray-500 mt-1">{stats?.transferCount || 0} переводов</div>
        </div>
        
        <div className="p-5 rounded-2xl glass border border-red-500/20 bg-gradient-to-br from-red-500/10 to-red-600/5">
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className="text-xs text-gray-500 mt-1">{stats?.burnCount || 0} сжиганий</div>
        </div>
        
        <div className="p-5 rounded-2xl glass border border-purple-500/20 bg-gradient-to-br from-purple-500/10 to-purple-600/5">
          <div className="flex items-center gap-2 mb-3">
            <Shield className="w-5 h-5 text-purple-400" />
            <span className="text-sm text-gray-400">Безопасность</span>
          </div>
          <div className="text-xl font-bold text-purple-400">Verified</div>
          <div className="text-xs text-gray-500 mt-1">OpenZeppelin</div>
        </div>
      </div>
      ...
      <div className="animate-fade-in-up stagger-3">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/orgs" className="text-sm text-cyan-400 hover:text-cyan-300 flex items-center gap-1">
            Все организации
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {balancesLoading ? (
            [...Array(2)].map((_, i) => (
              <div key={i} className="p-6 rounded-2xl glass border border-gray-800/50 animate-pulse">
                <div className="h-16 bg-gray-800 rounded-xl mb-4" />
            ...
            balances?.balances?.map((org: any) => {
              ...
              return (
                ...
                >
                  {/* Background gradient */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${colorClass === 'cyan' ? 'from-cyan-500/5 to-transparent' : 'from-purple-500/5 to-transparent'} opacity-0 group-hover:opacity-100 transition-opacity`} />
                  
                  <div className="relative flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className={`text-3xl p-3 rounded-xl bg-gradient-to-br ${colorClass === 'cyan' ? 'from-cyan-500/20 to-cyan-600/10' : 'from-purple-500/20 to-purple-600/10'}`}>
                        {orgMeta?.icon || '🏢'}
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold text-white group-hover:text-gradient-main">
                          {org.organization}
                        </h3>
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <ExternalLink className={`w-5 h-5 ${colorClass === 'cyan' ? 'text-cyan-400' : 'text-purple-400'}`} />
                    </div>
                  </div>
                  ...
                  <div className="relative flex items-end justify-between">
                    <div>
                      ...
                      <div className={`text-3xl font-bold ${colorClass === 'cyan' ? 'text-cyan-400' : 'text-purple-400'}`}>
                        {org.balance} <span className="text-lg text-gray-500">KSC</span>
                      </div>
                    </div>
                    
                    <div className="text-right">
                      <div className="text-xs text-gray-500 mb-2">Доля</div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-800/50 flex items-center justify-between text-sm">
                    <span className="text-gray-500">Перейти к управлению →</span>
                    <span className={`px-2 py-1 rounded-full text-xs ${colorClass === 'cyan' ? 'bg-cyan-500/10 text-cyan-400' : 'bg-purple-500/10 text-purple-400'}`}>
                      Активно
                    </span>
                  </div>
                </Link>
